              ...
              {s.wants.mentioned_properties.map((mp, i) => {
                ...
                return p ? (
                  <Link key={i} to="/properties/$propertyId" params={{ propertyId: p.id }}
                    ...
                    {label} <span className="text-[10px] text-muted-foreground capitalize">· {mp.status}</span>
                  </Link>
                ) : (
                  <span key={i} className="rounded-full bg-muted px-2 py-0.5 text-[11px]">{label} <span className="text-[10px] text-muted-foreground capitalize">· {mp.status}</span></span>
                );
              })}
            </ul>
          </div>
                ...
                <li key={i} className="rounded-lg border border-border p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{m.price} · {m.availability}</p>
                    </div>
