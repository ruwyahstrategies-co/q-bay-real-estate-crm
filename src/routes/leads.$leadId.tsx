              ...
              {files.map((f) => (
                <Card key={f.id} className="flex items-center justify-between gap-3 py-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{f.category} Â· {f.processing_status}</p>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <Card key={t.id} className="flex items-center justify-between gap-3 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Due {fmtDateTime(t.due_at)} Â· {t.priority}</p>
                    </div>
                  </div>
                  <button
                    className="rounded-md p-1.5 hover:bg-muted text-destructive"
...
function ActivityTimeline({
  ...
  const events: ActivityEvent[] = [
    ...
    ...interactions.map((i) => ({
      ...
      label: `Interaction Â· ${i.interaction_type.replace(/_/g, " ")}`,
      sub: i.subject ?? undefined,
    })),
    ...tasks.map((t) => ({ ts: t.created_at, label: `Task created Â· ${t.title}` })),
    ...tasks.filter((t) => t.completed_at).map((t) => ({ ts: t.completed_at!, label: `Task completed Â· ${t.title}` })),
    ...files.map((f) => ({ ts: f.created_at, label: `File uploaded Â· ${f.filename}` })),
              ...
              return (
                <li key={it.id} className="flex items-center justify-between gap-3 rounded-md border border-border p-2 text-xs">
                  <Link to="/properties/$propertyId" params={{ propertyId: p.id }} className="hover:underline">
                    {p.reference_code ? `${p.reference_code} Â· ` : ""}{p.title}
                  </Link>
