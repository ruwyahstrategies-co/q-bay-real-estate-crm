...
import {
  useOwner,
  useOwnerProperties,
  useOwnerDevelopments,
  useOwnerTransactions,
} from "@/hooks/use-owners";
import { useOwnerSubmissions } from "@/hooks/use-submissions";
...
import type { OwnerContract, PropertySubmission } from "@/lib/db";
...
function OwnerProfilePage() {
  ...
  const { data: submissions = [] } = useOwnerSubmissions(ownerId);
  ...
  const [activeSubmission, setActiveSubmission] = useState<PropertySubmission | null>(null);
  ...
  if (!owner)
    return (
      <AppShell>
        <EmptyState title="Owner not found" description="This owner may have been deleted." />
      </AppShell>
    );
  ...
  const upcomingFollowUp =
    tasks
      ...
      .find((t) => new Date(t.due_at!).getTime() >= now) ??
    ...
    <AppShell>
      <PermissionGate module="owners" action="view" page>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-mono uppercase tracking-wide">
                    {owner.code ?? "--"}
                  </span>
                  {owner.is_developer && (
                    <span className="flex items-center gap-1 rounded-full bg-pastel-green px-2 py-0.5 text-[10px]">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span>
                    Last contacted: {lastContacted ? fmtDateTime(lastContacted) : "Never"}
                  </span>
                  {upcomingFollowUp && (
                    <span>Next follow-up: {fmtDateTime(upcomingFollowUp.due_at)}</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <Card>
            ...
            <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
              ...
              <dd>{owner.is_developer ? "Developer" : "Individual"}</dd>
              <dt className="text-muted-foreground">Created</dt>
              <dd>{fmtDate(owner.created_at)}</dd>
              <dt className="text-muted-foreground">Last updated</dt>
              <dd>{fmtDate(owner.updated_at)}</dd>
            </dl>
            {canEdit && (
              <p className="mt-3 text-[11px] text-muted-foreground">
                Edit contact details from the Owners list.
              </p>
            )}
          </Card>
          ...
          <Card>
            <h4 className="text-sm font-semibold">
              Sold / rented / other listings ({closedListings.length})
            </h4>
          ...
          <Card className="md:col-span-2">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-sm font-semibold">
                Website / Listing Submissions ({submissions.length})
              </h4>
            </div>
            {submissions.length === 0 ? (
              <p className="mt-2 text-xs text-muted-foreground">
                Nothing submitted through List Your Property yet.
              </p>
            ) : (
              <div className="mt-3 space-y-2">
                {submissions.map((s) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-3 rounded-lg border border-border bg-canvas p-3 text-left hover:bg-muted/60"
                  >
                    <div>
                      <p className="text-sm font-medium">
                        {s.property_type ?? "Property"} · {s.location ?? "Location not given"}
                      </p>
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        Submitted {fmtDateTime(s.created_at)} · {s.source ?? "website"} ·{" "}
          ...
          <Card className="md:col-span-2">
            ...
            {transactions.length === 0 ? (
              <p className="mt-2 text-xs text-muted-foreground">
                No transactions recorded against this owner's properties yet.
              </p>
            ) : (
              <ul className="mt-3 space-y-2">
                {transactions.map((t: any) => (
                  ...
                  >
                    <span>
                      {t.properties?.reference_code ? `${t.properties.reference_code} · ` : ""}
                      {t.properties?.title ?? "Property"} - {t.transaction_type}
                    </span>
                    <span className="text-muted-foreground">
                      {fmtMoney(t.transaction_value, t.currency)} · {t.status}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <Card className="md:col-span-2">
            ...
            {contracts.length === 0 ? (
              <EmptyState
                ...
                description="Generate one from a linked property."
              />
            ) : (
              <div className="mt-3 space-y-2">
                {contracts.map((c) => (
                  <div
                    key={c.id}
                    className="flex items-center justify-between gap-3 rounded-lg border border-border bg-canvas p-3"
                  >
                    <div>
                      <p className="text-sm font-medium capitalize">
                        {c.purpose} contract{" "}
                        {c.properties
                          ? `· ${c.properties.reference_code ?? c.properties.title}`
                          : ""}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {c.status} · {fmtMoney(c.amount, c.currency)}{" "}
                    ...
                    <div className="flex items-center gap-2">
                      {c.generated_html && (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            openContractPdf(c.generated_html!, contractTitle(null, owner))
                          }
                        >
                          View PDF
                        </Button>
            ...
            {canUpload && (
              <div className="mt-3">
                <UploadDropzone
                  ...
                  description="Contracts, ID, POA, etc."
                  categoryKey="owner_documents"
                  ownerId={owner.id}
                />
              </div>
            )}
          ...
          <Card className="md:col-span-2">
            ...
            {interactions.length === 0 ? (
              <EmptyState
                ...
                description="Use Log Contact to record calls, meetings or notes."
              />
            ) : (
              <div className="mt-3 space-y-2">
                {interactions.map((i) => (
                  <div
                    key={i.id}
                    className="flex items-start justify-between gap-3 rounded-lg border border-border bg-canvas p-3"
                  >
        ...
        <SubmissionDetailDrawer
          ...
          onOpenChange={(v) => !v && setActiveSubmission(null)}
