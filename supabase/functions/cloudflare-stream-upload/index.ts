...
Deno.serve(async (req) => {
  ...
  if (!accountId || !apiToken) {
    return json({
      error: "Cloudflare Stream is not configured yet. Missing CLOUDFLARE_ACCOUNT_ID and/or CLOUDFLARE_STREAM_API_TOKEN as Edge Function secrets.",
      configured: false,
    }, 501);
  }
  ...
  if (!cfResp.ok || !cfData?.success) {
    const message = cfData?.errors?.[0]?.message ?? `Cloudflare Stream request failed (HTTP ${cfResp.status})`;
