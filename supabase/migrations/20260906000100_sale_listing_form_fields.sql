-- Q-Bay Sale Listing Form: fields the paper/Excel form captures that the
-- properties table could not yet represent. Everything else on the form
-- (owner, agent, reference, development, media, documents, notes, website
-- publishing, contract link) already exists, so nothing is rebuilt here.
...
alter table public.properties drop constraint if exists properties_parking_spaces_check;
alter table public.properties
  add constraint properties_parking_spaces_check
  check (parking_spaces is null or parking_spaces >= 0);

comment on column public.properties.furnishing_status is 'FF = fully furnished, SF = semi furnished, UF = unfurnished (Q-Bay Sale Listing Form)';

-- Owner ID number is on the client/owner block of the same form.
alter table public.owners add column if not exists id_number text;

-- Republish the public listing feed with the new fields so the website can
-- show them in the property specification table. The return type changes, so
-- the function has to be dropped rather than replaced.
...
returns table (
  ...
  tower_name text, floor_number text, unit_number text,
  parking_spaces integer, furnishing_status text
)
language sql
stable
security definer
